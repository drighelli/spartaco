main <- function(x, K, R, coordinates, Dist,
                 traceRatio = 10,
                 max.iter = 1000,
  ...
  cur.Cs <- best.Ds <- sample(seq_len(K), size = nrow(x), replace = TRUE)
  cur.Ds <- best.Cs <- sample(seq_len(R), size = ncol(x), replace = TRUE)#kmeans(t(x), centers = R)$clust
  ...
  for(i in seq_len(max.iter)[-1]){
    ...
    for(r in seq_len(R)){
      ...
      for(k in 1:K){
        ...
        estimation.parameters <- Estimate.Cocluster.Parameters.constraint.trace(x = x[cur.Cs == k, cur.Ds == r],
    ...
    if(i %% ifelse(K > 1, 2, 1) == 0){
      cur.ds <- MetropolisAllocation(x = x, Uglob = Uglob, Dglob = Dglob,
                                     ...
                                     maxit = metropolis.iterations, min.obs = 3)
